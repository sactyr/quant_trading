...
# Source shared config and trading functions -----------------------------------
...
compute_md5 <- function(df) {
  concat <- paste(df$date, df$open, df$high, df$low, df$close, sep = "|")
...
#' @param non_trading_dates Character vector of known non-trading dates
