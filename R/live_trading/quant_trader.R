...
tryCatch({
  ...
  log_info("Step 1a: Confirming IBKR session...")
  ...
  log_info("Step 1b: Warming up iserver connection...")
  
  tryCatch({
    status <- ibkr_auth_status()
    if (!isTRUE(status$authenticated)) {
      stop("iserver is not authenticated — restart the gateway and re-authenticate.")
    }
    log_info("iserver authenticated and connected.")
  }, error = function(e) {
    log_error("iserver warmup failed: {e$message}")
    stop(e)
  })
  
  log_info("Step 1c: Initialising portfolio session...")
  
  tryCatch({
    ibkr_portfolio_accounts()
    log_info("Portfolio session initialised.")
  }, error = function(e) {
    log_error("Portfolio session initialisation failed: {e$message}")
    stop(e)
  })
  ...
  today                <- Sys.Date()
  expected_latest_date <- last_trading_day(today, non_trading_dates)
  ...
  # Step 5: Fetch IBKR positions -------------------------------------------------
  
  log_info("Step 5: Fetching IBKR positions...")
  
  ibkr_positions <- tryCatch({
    positions <- ibkr_portfolio_positions(ibkr_account_id)
    if (nrow(positions) > 0) {
  ...
  ibkr_cash <- tryCatch({
    summary <- ibkr_portfolio_summary(ibkr_account_id)
