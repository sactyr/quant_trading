...
single_window_results <- map(etf_symbols, function(symbol) {
  ...
  summary_metrics <- get_performance_metrics(
    results_list
    ,initial_equity = init_equity
    ,tax_regime     = "current"
    ,marginal_rate  = marginal_tax_rate
    ,inflation_pa   = cgt_inflation_pa
  )
...
output_dir <- file.path(
  quant_dir, "outputs", "backtesting",
  format(Sys.Date(), "%Y-%m")
)

dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

## Run Monte Carlo per regime and ETF -------------------------------------

all_etf_monte_carlo <- map(tax_regimes, function(tax_regime) {
  
  message("\n========== Tax regime: ", tax_regime, " ==========")
  
  map(etf_symbols, function(symbol) {
    ...
    for (sl in stop_losses) {
      ...
      chunk_file <- file.path(
        output_dir
        ,paste0(symbol, "_SL", sl * 100, "_", tax_regime, "_monte_carlo_summary.rds")
      )
      
      if (file.exists(chunk_file)) {
        message("Skipping SL ", sl * 100, "% for ", symbol, " (", tax_regime, ") - already saved, loading from disk")
      ...
      message("Saved chunk: SL", sl * 100, "% for ", symbol, " (", tax_regime, ")")
...
walk(tax_regimes, function(tax_regime) {
  walk(etf_symbols, function(symbol) {
    saveRDS(
      object = all_etf_monte_carlo[[tax_regime]][[symbol]]
      ,file  = file.path(
        output_dir
        ,paste0(symbol, "_", tax_regime, "_monte_carlo_summary.rds")
      )
    )
  })
})
...
all_etf_monte_carlo <- map(tax_regimes, function(tax_regime) {
  map(etf_symbols, function(symbol) {
    ...
    if (!file.exists(target_file)) {
      warning("No combined file found for ", symbol, " regime ", tax_regime, " - skipping")
      return(NULL)
    }
...
aggregate_caps <- function(etf_monte_carlo) {
  map(names(etf_monte_carlo), function(symbol) {
    etf_monte_carlo[[symbol]] %>%
    ...
    set_names(names(etf_monte_carlo))
}

agg_summaries <- map(tax_regimes, function(tax_regime) {
  aggregate_caps(all_etf_monte_carlo[[tax_regime]])
}) %>%
  set_names(tax_regimes)


## Compare top strategy per ETF across regimes ----------------------------

regime_comparison <- map(etf_symbols, function(symbol) {
  map(tax_regimes, function(tax_regime) {
    agg_summaries[[tax_regime]][[symbol]] %>%
      slice_head(n = 1) %>%
      mutate(etf = symbol, regime = tax_regime)
  }) %>%
    bind_rows()
}) %>%
  bind_rows() %>%
  select(etf, regime, strategy_type, stop_loss, geo_mean_CAGR, CAPS) %>%
  arrange(etf, regime)

print(regime_comparison, n = Inf)


## Visualisation ----------------------------------------------------------

### The Efficient Frontier (one plot per ETF per regime) -----------------

walk(tax_regimes, function(tax_regime) {
  walk(names(agg_summaries[[tax_regime]]), function(symbol) {
    
    ranked_summary <- agg_summaries[[tax_regime]][[symbol]]
    ...
    p <- ggplot(
      ...
      labs(
        ...
        ,subtitle = paste("Tax regime:", tax_regime, "| Top 5 CAPS highlighted")
        ,x        = "Tail Risk (CVaR Drawdown)"
        ,y        = "Geometric Mean CAGR (post-tax)"
      ) +
      theme_minimal()
